...
def multi_fun0_detectColonySingleImage(image_trans_path, image_epi_path, image_label, configure_pool, varPool, index):
    image_trans_corrected, image_epi_corrected, all_contours, all_metadata = fun0_detectColonySingleImage(
...
# background_STD = configure_pool["background_STD"]
    ...
    start_time = time.time()
    print("Finish " + image_label +
          " image loading... (Execution time: " + str(time_dur) + ")")

    # crop the images
    image_trans_crop = crop_image(
        image_trans_raw, cropXMin, cropXMax, cropYMin, cropYMax)
    image_epi_crop = crop_image(
        image_epi_raw, cropXMin, cropXMax, cropYMin, cropYMax)
    height_crop, width_crop = image_trans_crop.shape[:2]

    # correct image
    image_trans_corrected = (image_trans_crop.astype(np.float32) / image_trans_calib.astype(
        np.float32)) * calib_contrast_trans_alpha + calib_contrast_trans_beta
    image_epi_corrected = image_epi_crop.astype(np.float32)
    image_epi_corrected[:, :, 0] = image_epi_corrected[:,
                                                       :, 0] / image_epi_calib_B.astype(np.float32)
    image_epi_corrected[:, :, 1] = image_epi_corrected[:,
                                                       :, 1] / image_epi_calib_G.astype(np.float32)
    image_epi_corrected[:, :, 2] = image_epi_corrected[:,
                                                       :, 2] / image_epi_calib_R.astype(np.float32)
...
# image_mask_bg = cv2.inRange(image_gray_first, 0, bg_gray_mean - background_STD * bg_gray_sd)
    ...
    start_time = time.time()
    print("Finish " + image_label +
          " background removal... (Execution time: " + str(time_dur) + ")")

    # gaussian blur and perform edge detection
    image_res_GB = cv2.GaussianBlur(image_res_bg, (5, 5), 0)
    upper = calculate_canny_upper(
        image_res_GB, size_subSample, canny_upper_percentile)
    ...
    start_time = time.time()
    print("Finish " + image_label +
          " first run of colony detection... (Execution time: " + str(time_dur) + ")")

    finalContours, finalDF = filterContours(
        contours, configure_pool, image_trans_corrected, image_epi_corrected, height_crop, width_crop)

    time_dur = round(time.time() - start_time, 2)
    start_time = time.time()
    print("Finish " + image_label +
          " colony filtering... (Execution time: " + str(time_dur) + ")")
    ...
    start_time = time.time()
    print("Finish " + image_label +
          " multi-colony segmentation... (Execution time: " + str(time_dur) + ")")

    post_finalContours_final = filterContours_final(
        post_finalContours, configure_pool, image_trans_crop, image_epi_crop, height_crop, width_crop)

    post_finalDF = getFinalData(post_finalContours_final, configure_pool,
                                image_trans_corrected, image_epi_corrected, height_crop, width_crop)
    time_dur = round(time.time() - start_time, 2)
    start_time = time.time()
    print("Finish " + image_label +
          " final data gathering... (Execution time: " + str(time_dur) + ")")
...
def fun1_runPlateQualityControl(image_trans_crop, image_epi_crop, post_finalContours, post_finalDF, image_label, configure_pool):
    ...
    time_dur = round(time.time() - start_time, 2)
    print("Finish " + image_label +
          " plate QC... (Execution time: " + str(time_dur) + ")")
    return True  # skip quality control
...
def fun2_pickColonyPilot(post_finalDF, num_of_colonies, image_label, configure_pool):
    ...
    time_dur = round(time.time() - start_time, 2)
    print("Finish " + image_label +
          " first run of colony selection... (Execution time: " + str(time_dur) + ")")
...
def fun3_runColonyQualityControl_group(eachGroupID, groupID_index, varPool, configure_pool, sample_config):
    ...
    if len(groupID_index) > 0:
        ...
        groupID_image_epi_list = [
            varPool.image_epi_corrected[i] for i in groupID_index]
        groupID_contour_list = [varPool.all_contours[i] for i in groupID_index]
        groupID_metadata_list = [varPool.all_metadata[i]
                                 for i in groupID_index]
        ...
        groupID_image_merge, groupID_image_heightStart = concatenateImages_gray(
            groupID_image_trans_list, groupID_label_list, spacing, fontSize, fontThickness)
...
def fun3_runColonyQualityControl(height_crop, width_crop, image_trans_crop, post_finalContours, post_finalDF, pick_choice, post_finalDF_PCA, image_label, configure_pool):
    ...
    final_pick, bad_pick = colonyQualityControl(height_crop, width_crop, image_trans_crop, colonyQC_imageScaleFactor, (width_crop * colonyQC_imageScaleFactor + colonyQC_imageWidthBias,
                                                ...
                                                colonyQC_colonyColumnNum, image_label, post_finalContours,
    ...
    time_dur = round(time.time() - start_time, 2)
    print("Finish " + image_label +
          " colony QC... (Execution time: " + str(time_dur) + ")")
...
def concatenateImages_gray(image_list, label_list, spacing, fontSize, fontThickness):
    ...
    for i in range(len(image_list)):
        ...
        cv2.putText(blankImage, eachLabel, (fontSize * 10, spacing - fontSize * 10),
                    cv2.FONT_HERSHEY_SIMPLEX, fontSize, (0, 0, 0), fontThickness)
...
def calculate_calib_image(trans_file_path, epi_file_path, configure_pool):
    ...
    for eachImage in trans_file_path:
        ...
        trans_image_list.append(crop_image(
            image_trans_tmp, cropXMin, cropXMax, cropYMin, cropYMax))
    for eachImage in epi_file_path:
        image_epi_tmp = cv2.imread(eachImage)
        epi_image_list.append(crop_image(
            image_epi_tmp, cropXMin, cropXMax, cropYMin, cropYMax))
    image_trans_calib = calculate_calib_background_gray(
        trans_image_list, gaussian_kernal, gaussian_iteration)
    image_epi_calib_B, image_epi_calib_G, image_epi_calib_R = calculate_calib_background_BGR(
        epi_image_list, gaussian_kernal, gaussian_iteration)
    np.savez(calib_parameter_PATH, image_trans_calib=image_trans_calib, image_epi_calib_B=image_epi_calib_B,
             image_epi_calib_G=image_epi_calib_G, image_epi_calib_R=image_epi_calib_R)
...
def calculate_calib_background_BGR(image_list, gaussian_kernal, gaussian_iteration):
    ...
    for i in range(gaussian_iteration):
        gaussian_input_B = cv2.GaussianBlur(
            gaussian_input_B, gaussian_kernal, 0)
        gaussian_input_G = cv2.GaussianBlur(
            gaussian_input_G, gaussian_kernal, 0)
        gaussian_input_R = cv2.GaussianBlur(
            gaussian_input_R, gaussian_kernal, 0)
...
def calculate_canny_upper(image_res_GB, size_subSample, percentile):
    ...
    image_res_GB_flatten_sub_noZero = np.ma.masked_equal(
        image_res_GB_flatten_sub, 0).compressed()
...
def calculate_background_GMM(image_gray_first_con, size_subSample, mean_empirical):
    ...
    index_Gray = [e for e in range(len(clf_Gray.means_)) if clf_Gray.means_[
        e] == max(clf_Gray.means_)][0]
...
def getFinalData(contours, configure_pool, image_trans_crop, image_epi_crop, height_crop, width_crop):
    ...
    for contour in contours:
        ...
        dists.sort()
        radius = (dists[int((len(dists) - 1)/2)] +
                  dists[int(len(dists)/2)]) / 2
        perim = cv2.arcLength(contour, True)
        circularity = (4*np.pi*area) / (perim**2)
        hullArea = cv2.contourArea(cv2.convexHull(contour))
        convexityRatio = area/hullArea
        denom = np.sqrt((2*moms['mu11'])**2) + \
            ((moms['mu20'] - moms['mu02'])**2)
        eps = .01
        inertiaRatio = 1
        if (denom > eps):
        ...
        image_trans_mask = cv2.bitwise_and(image_trans_crop[topY: (topY + height + 1), topX: (topX + width + 1)],
                                           image_trans_crop[topY: (
                                               topY + height + 1), topX: (topX + width + 1)],
        ...
        image_epi_mask = cv2.bitwise_and(image_epi_crop[topY: (topY + height + 1), topX: (topX + width + 1)],
                                         image_epi_crop[topY: (
                                             topY + height + 1), topX: (topX + width + 1)],
                                         mask=blackImg[topY: (topY + height + 1), topX: (topX + width + 1)])
        gray_list = np.ma.masked_equal(
            image_trans_mask.flatten(), 0).compressed()
        Bepi_list = np.ma.masked_equal(
            image_epi_mask[:, :, 0].flatten(), 0).compressed()
        Gepi_list = np.ma.masked_equal(
            image_epi_mask[:, :, 1].flatten(), 0).compressed()
        Repi_list = np.ma.masked_equal(
            image_epi_mask[:, :, 2].flatten(), 0).compressed()
...
def process_segmentation(image_subContour, image_subContour_binary, configure_pool):
    ...
    local_maxi_coord = peak_local_max(distance, footprint=np.ones(
        (randowWalker_maxiSize, randowWalker_maxiSize)), labels=image_subContour_binary)
    local_maxi = np.zeros(np.shape(distance), dtype=bool)
    ...
    labels_rw = random_walker(
        image_subContour, markers, beta=randomWalker_beta, mode=randomWalker_method)
    return (labels_rw)
...
def postprocess_contours(finalDF, finalContours, image_trans_crop, configure_pool):
    ...
    for i in range(len(finalDF)):
        ...
        if ifPostProcessFlag == 0:
            ...
            cv2.drawContours(image_subContour_binary, [
                             modifiedContour], 0, 255, -1)
            cv2.drawContours(image_subContour_binary, [
                             modifiedContour], 0, 255, 1)
            image_subContour = cv2.bitwise_and(image_trans_copy[yStart:(yEnd + 1), xStart:(xEnd + 1)].astype(np.uint8),
                                               image_trans_copy[yStart:(
                                                   yEnd + 1), xStart:(xEnd + 1)].astype(np.uint8),
            ...
            post_contours = postprocess_segmentation(
                xStart, xEnd, yStart, yEnd, image_subSegmentation, image_subContour.copy())
            post_contours_filtered = post_filterContours(
                post_contours, configure_pool)
...
def plateQualityControl(image_trans_crop, resize_factor, main_window_size, confirm_window_size, image_label, num_of_colony, textSizeLarge, textSizeSmall, textSizeButton):
    ...
    while True:
        ...
        resize_image = cv2.resize(image_trans_crop, (int(
            width_crop * resize_factor), int(height_crop * resize_factor)))
        final_image = resize_image
        current_image = Image.fromarray(final_image)
        imgtk = ImageTk.PhotoImage(image=current_image)
        panel = Label(root, image=imgtk)
        panel.grid(row=0, column=0, columnspan=17,
                   sticky=W+E+N+S, padx=5, pady=5)
        but_test = Label(root, text="Keep this plate?",
                         font=('Times', textSizeLarge))
        but_test.grid(row=1, column=7, columnspan=3,
                      sticky=W + E + S, padx=5, pady=0)
        but_test2 = Label(root, text="(total number of colonies on this plate: " +
                          str(num_of_colony) + ")", font=('Times', textSizeSmall))
        but_test2.grid(row=2, column=5, columnspan=7,
                       sticky=W + E + S, padx=5, pady=0)
        ...
        def click_Yes(varPool):
            ...
            top.title('Confirm')
            top.geometry(
                "x".join([str(confirm_size[0]), str(confirm_size[1])]))
            confirm_test = Label(top, text="Keep this plate?",
                                 font=('Times', textSizeLarge))
            confirm_test.grid(row=0, column=0, columnspan=5,
                              sticky=W + E + S, padx=10, pady=10)
            ...
            confirm_button = Button(top, text='Yes', command=click_confirm_Yes_with_arg,
                                    foreground="green", font=('Times', textSizeButton))
        ...
        def click_No(varPool):
            ...
            confirm_button = Button(top, text='No', command=click_confirm_No_with_arg,
                                    foreground="red", font=('Times', textSizeButton))
            confirm_button.grid(row=1, column=2, padx=1, pady=1, sticky=W+E+N)
        click_No_with_arg = partial(click_No, globalVar)
        but_yes = Button(root, text="Yes", command=click_Yes_with_arg,
                         foreground="green", font=('Times', textSizeButton))
        but_no = Button(root, text="No", command=click_No_with_arg,
                        foreground="red", font=('Times', textSizeButton))
...
def drawPinSite(image_all_contours, contours, pixel):
    image_output = cv2.cvtColor(
        image_all_contours.astype(np.uint8), cv2.COLOR_GRAY2BGR)
    for contour in contours:
        moms = cv2.moments(contour)
        x = int((moms['m10'])/(moms['m00']))
        y = int((moms['m01'])/(moms['m00']))
...
def concat_metadata(metadata_list, heightStart):
    ...
    for i in range(len(metadata_list)):
        ...
        tmp_metadata["Y_concat"] = [
            e + tmp_Height for e in list(tmp_metadata["Y"])]
...
def farthest_points_parallel(data, n, threadIndex, outputObject):
    ...
    while r_old != r:
        ...
        for i in range(n):
            no_i = r[:]
            no_i.pop(i)
            cols_in_play = np.asarray(range(dist_mat.shape[1]))[np.newaxis, :][:, filter(
                lambda n: n not in no_i, range(dist_mat.shape[1]))]
            mm = dist_mat[no_i, :][:, filter(
                lambda n: n not in no_i, range(dist_mat.shape[1]))]
...
def farthest_points(data, n):
    ...
    while r_old != r:
        ...
        for i in range(n):
            no_i = r[:]
            no_i.pop(i)
            cols_in_play = np.asarray(range(dist_mat.shape[1]))[np.newaxis, :][:, list(
                filter(lambda n: n not in no_i, range(dist_mat.shape[1])))]
            mm = dist_mat[no_i, :][:, list(
                filter(lambda n: n not in no_i, range(dist_mat.shape[1])))]
            max_min_dist = np.argmax(np.min(mm, 0))
            r[i] = cols_in_play[0, :][max_min_dist]
...
def pickColonyFirst(finalDF, num_of_pick, iteration):
    ...
    for robust_iter in range(iteration):
        ...
        print("Finish farthest points iteration " + str(robust_iter) +
              "... (Execution time: " + str(time_dur) + ")")
        startTime = time.time()
...
def generateContourSubImage_QC(image_trans_crop, contour, midpoint, segment_bias, final_size, pixel, label, fontScale, thickness):
    image_output = cv2.cvtColor(
        image_trans_crop.astype(np.uint8), cv2.COLOR_GRAY2BGR)
    cv2.drawContours(image_output, [contour], 0, [0, 0, 0], pixel)
    ...
    if xLength > yLength:
        if (xLength - yLength) % 2 == 0:
            ...
            xEnd_f = min(xEnd, width_crop - 1)
            xStart_f = max(xStart, 0)
            yEnd_f = min(yEnd + int((xLength - yLength) / 2) +
                         1, height_crop - 1)
        ...
        if (yLength - xLength) % 2 == 0:
            ...
            yEnd_f = min(yEnd, height_crop - 1)
            yStart_f = max(yStart, 0)
            xEnd_f = min(xEnd + int((yLength - xLength) / 2) +
                         1, width_crop - 1)
            xStart_f = max(xStart - int((yLength - xLength) / 2), 0)
    image_sub_contour = image_output[int(yStart_f):int(
        yEnd_f + 1), int(xStart_f):int(xEnd_f + 1)]
...
def addRedXtoImage(image_to_add, pixel):
    if len(image_to_add.shape) == 2:
        image_output = cv2.cvtColor(
            image_to_add.astype(np.uint8), cv2.COLOR_GRAY2BGR)
    else:
        image_output = image_to_add.copy()
    pic_size = image_to_add.shape[0]
    cv2.line(image_output, (0, 0), (pic_size - 1,
             pic_size - 1), [0, 0, 255], pixel)
    cv2.line(image_output, (pic_size - 1, 0),
             (0, pic_size - 1), [0, 0, 255], pixel)
    return image_output
...
def colonyQualityControl(height_crop, width_crop, image_trans_crop, resize_factor, main_window_size, confirm_window_size, colony_show_bias,
    ...
    globalVar.pick_finalContours = [
        post_finalContours[i] for i in globalVar.finalPick]
    globalVar.image_picked_contours = drawContour(
        image_trans_crop, globalVar.pick_finalContours, colonyQC_colonyContourPixel)
    globalVar.image_picked_contours_label = drawContourLabel(globalVar.image_picked_contours, globalVar.pick_finalDF, range(
        1, len(globalVar.finalPick) + 1), colonyQC_colonyLabelSize, colonyQC_colonyLabelThickness)
    ...
    globalVar.remaining_colonies = len(
        post_finalContours) - len(globalVar.finalPick)
    ...
    while True:
        ...
        resize_image = cv2.resize(cv2image, (int(
            width_crop_merge * resize_factor * 18 / 19), int(height_crop_merge * resize_factor * 18 / 19)))
        ...
        for i in range(len(globalVar.finalPick)):
            ...
            globalVar.pick_colony_bad_tk.append(
                image_to_tk(addRedXtoImage(tmpSub, 2)))
            globalVar.pick_colony_tk.append(image_to_tk(tmpSub))

        def show_whole_image(varPool):

            frm_whole = Frame(root, width=str(
                int(width_crop * resize_factor)), height=str(int(height_crop * resize_factor)))
            canvas_whole = Canvas(frm_whole, width=str(int(
                width_crop * resize_factor) - 30), height=str(int(height_crop * resize_factor)))
            ...
            canvas_whole.create_window(
                (0, 0), window=frm_image_whole, anchor='nw')
            ...
            panel.grid(row=0, column=0, columnspan=18,
                       sticky=W+E+N+S, padx=5, pady=5)

            but_test = Label(root, text="View all selected colonies on plates", font=(
                'Times', colonyQC_TextSizeLarge))
            but_test.grid(row=1, rowspan=2, column=5,
                          columnspan=9, sticky=W + E, padx=5, pady=10)
            ...
            but_continue = Button(root, text="Continue", command=click_continue_with_arg,
                                  foreground="black", font=('Times', colonyQC_TextSizeButton))
            but_continue.grid(row=3, column=9, sticky=W+E+N, padx=5, pady=5)

        def show_ind_image(varPool):
            frm = Frame(root, width=str(int(width_crop * resize_factor)),
                        height=str(int(height_crop * resize_factor)))
            canvas = Canvas(frm, width=str(int(
                width_crop * resize_factor) - 30), height=str(int(height_crop * resize_factor)))
            frm_but = Frame(canvas)
            myscrollbar = Scrollbar(
                frm, orient="vertical", command=canvas.yview)
            ...
            def button_fun(args):
                ...
                if varPool_1.colony_button_flag[k] == 0:
                    ...
                    colony_button_list[k].config(
                        image=varPool_1.pick_colony_bad_tk[k])
                else:
                    varPool_1.colony_button_flag[k] = 0
                    colony_button_list[k].config(
                        image=varPool_1.pick_colony_tk[k])
            for i in range(len(varPool.finalPick)):
                button_fun_with_arg = partial(button_fun, (i, varPool))
                if varPool.colony_button_flag[i] == 0:
                    tmpButton = Button(
                        frm_but, command=button_fun_with_arg, image=varPool.pick_colony_tk[i])
                else:
                    tmpButton = Button(
                        frm_but, command=button_fun_with_arg, image=varPool.pick_colony_bad_tk[i])
                tmpButton.grid(row=int(i / 6), column=int(3 * (i %
                               6)), columnspan=3, padx=7, pady=10)
            ...
            but_text1 = Label(root, text="Please select bad colonies", font=(
                'Times', colonyQC_TextSizeMid))
            but_text1.grid(row=1, column=5, columnspan=8,
                           sticky=W + E + S, padx=5, pady=0)
            but_text2 = Label(root, text="(remaining colonies for re-select: " + str(
                varPool.remaining_colonies) + ")", font=('Times', colonyQC_TextSizeSmall))
            but_text2.grid(row=2, column=5, columnspan=8,
                           sticky=W + E + N, padx=5, pady=0)
            ...
            def click_reselect(varPool_1):
                tmpPick = [varPool_1.finalPick[i] for i in range(
                    len(varPool_1.finalPick)) if varPool_1.colony_button_flag[i] == 0]
                if len(tmpPick) == 0:
                    time.sleep(0.1)
                    top = Toplevel()
                    top.title('Warning')
                    ...
                    warning_test = Label(
                        top, text="At least one colony should be kept for re-pick", font=('Times', colonyQC_TextSizeMid))
                    warning_test.grid(
                    ...
                    def click_warning_OK():
                        time.sleep(0.1)
                        top.destroy()
                    warning_button = Button(top, text='OK', command=click_warning_OK, foreground="black", font=(
                        'Times', colonyQC_TextSizeButton))
                    warning_button.grid(
                        row=1, column=2, padx=1, pady=1, sticky=W+E+N)
                else:
                    toPick = min(varPool_1.remaining_colonies,
                                 varPool_1.num_of_pick - len(tmpPick))
                    varPool_1.remaining_colonies -= toPick
                    varPool_1.bad_colonies += [varPool_1.finalPick[i] for i in range(
                        len(varPool_1.finalPick)) if varPool_1.colony_button_flag[i] == 1]
                    if toPick != 0:
                        ...
                        varPool_1.pick_finalDF.index = range(
                            len(varPool_1.finalPick))
                        varPool_1.image_picked_contours = drawContour(
                            image_trans_crop, varPool_1.pick_finalContours, colonyQC_colonyContourPixel)
                        ...
                        for i in range(len(varPool_1.finalPick)):
                            ...
                            varPool_1.pick_colony_bad_tk.append(
                                image_to_tk(addRedXtoImage(tmpSub, 2)))
                            varPool_1.pick_colony_tk.append(
                                image_to_tk(tmpSub))
                        ...
                        warning_test = Label(
                            top, text="There are no more colonies for re-pick", font=('Times', colonyQC_TextSizeMid))
                        warning_test.grid(
            ...
            def click_finish(varPool_1):
                ...
                top.geometry(
                    "x".join([str(confirm_size[0]), str(confirm_size[1])]))
                confirm_test = Label(top, text="Are you sure to pick these colonies?", font=(
                    'Times', colonyQC_TextSizeMid))
                confirm_test.grid(row=0, column=0, columnspan=5,
                                  sticky=W + E + S, padx=10, pady=10)
                ...
                def click_confirm_Yes(varPool_2):
                    ...
                    top.destroy()
                    root.destroy()
                    varPool_2.outputPick = [varPool_2.finalPick[i] for i in range(
                        len(varPool_2.finalPick)) if varPool_2.colony_button_flag[i] == 0]
                    varPool_2.breakFlag = 1
                    varPool_2.bad_colonies += [varPool_2.finalPick[i] for i in range(
                        len(varPool_2.finalPick)) if varPool_2.colony_button_flag[i] == 1]
                click_confirm_Yes_with_arg = partial(
                    click_confirm_Yes, varPool_1)

                def click_confirm_No():
                    time.sleep(0.1)
                    top.destroy()
                confirm_button = Button(top, text='No', command=click_confirm_No, foreground="red", font=(
                    'Times', colonyQC_TextSizeButton))
                confirm_button.grid(row=1, column=1, padx=1,
                                    pady=1, sticky=W+E+N)
                confirm_button = Button(top, text='Yes', command=click_confirm_Yes_with_arg,
                                        foreground="green", font=('Times', colonyQC_TextSizeButton))
                confirm_button.grid(row=1, column=3, padx=1,
                                    pady=1, sticky=W+E+N)
            click_finish_with_arg = partial(click_finish, varPool)
            but_back = Button(root, text="View plate", command=click_back_with_arg,
                              foreground="black", font=('Times', colonyQC_TextSizeButton))
            but_re = Button(root, text="Re-pick", command=click_reselect_with_arg,
                            foreground="red", font=('Times', colonyQC_TextSizeButton))
            but_finish = Button(root, text="Finish", command=click_finish_with_arg,
                                foreground="green", font=('Times', colonyQC_TextSizeButton))
            but_back.grid(row=3, column=4, columnspan=3,
                          sticky=W+E+N, padx=5, pady=5)
            but_re.grid(row=3, column=8, columnspan=3,
                        sticky=W+E+N, padx=5, pady=5)
            but_finish.grid(row=3, column=12, columnspan=3,
                            sticky=W+E+N, padx=5, pady=5)
...
def readFileList(input_dir):
    ...
    if os.path.isfile(input_dir + "/image_processed.txt"):
        f = open(input_dir + "/image_processed.txt", "r")
        data = f.readlines()
        f.close()
        for e in data:
    ...
    for e in image_second.keys():
        ...
        tmpList.sort()
        image_trans_list.append(input_dir + "/" + e +
                                "_" + str(tmpList[0]) + ".bmp")
        image_epi_list.append(input_dir + "/" + e +
                              "_" + str(tmpList[1]) + ".bmp")
...
def drawOutputContoursWhole(image_trans, image_epi, all_contours, pick_index, metadataDF, pixel, fontScale, thickness, pinSize):
    image_trans_BGR = cv2.cvtColor(
        image_trans.astype(np.uint8), cv2.COLOR_GRAY2BGR)
...
def saveOutputs_pickingOptimization(globalOutput, total_image, configure_pool, output_dir, groupID_index):
    for i in range(total_image):
        ...
        for j in range(len(tmpPickX)):
            f.writelines([str(tmpPickX[j]) + "," +
                         str(tmpPickY[j]) + os.linesep])
        f.close()
        output_image_trans, output_image_epi = drawOutputContoursWhole(
            tmp_image_trans, tmp_image_epi, tmp_all_contours, tmp_pick_index, tmp_metadataDF, 2, 0.7, 2, 2)
        cv2.imwrite(output_dir + "/" + tmp_image_label +
                    "_Image_colony_trans_pick.jpg", output_image_trans)
        cv2.imwrite(output_dir + "/" + tmp_image_label +
                    "_Image_colony_epi_pick.jpg", output_image_epi)
        output_image_trans_all = drawContour(
            tmp_image_trans, tmp_all_contours, 2)
        output_image_trans_all_pin = drawPinSite(
            output_image_trans_all, tmp_all_contours, 2)
        cv2.imwrite(output_dir + "/" + tmp_image_label +
                    "_Image_colony_trans.jpg", output_image_trans_all_pin)
        tmp_metadataDF.to_csv(
            output_dir + "/" + tmp_image_label + "_Metadata_all.csv")
        np.save(output_dir + "/" + tmp_image_label +
                "_Contours_all.npy", tmp_all_contours)
    timeStamp = time.time()
    timeValue = datetime.datetime.fromtimestamp(timeStamp)
    timeFormat = timeValue.strftime('%Y%m%d_%H%M%S_%f')

    f = open(output_dir + "/pickingOptimization." +
             timeFormat + ".merge.obj", 'wb')
...
def modifyOutputObject_colonyDetection(globalOutput, total_image, configure_pool):
    for i in range(total_image):
        tmp_metadataDF = globalOutput.all_metadata[i]
        tmp_all_contours = globalOutput.all_contours[i]
        tmp_metadataDF["X_rawImage"] = tmp_metadataDF["X"] + \
            configure_pool["cropXMin"]
        tmp_metadataDF["Y_rawImage"] = tmp_metadataDF["Y"] + \
            configure_pool["cropYMin"]
        tmp_metadataDF["plate_barcode"] = [
            globalOutput.image_label[i], ] * len(tmp_all_contours)
...
def saveOutputs_colonyDetection(globalOutput, total_image, configure_pool, output_dir):
    for i in range(total_image):
        ...
        tmp_image_epi = globalOutput.image_epi_corrected[i]
        #tmp_all_contours = globalOutput.all_contours[i]
        tmp_all_contours = np.empty(len(globalOutput.all_contours[i]), dtype=object)
        for j in range(len(globalOutput.all_contours[i])):
                tmp_all_contours[j] = globalOutput.all_contours[i][j]
        tmp_image_label = globalOutput.image_label[i]
        tmp_metadataDF = globalOutput.all_metadata[i]
        output_image_trans_all = drawContour(
            tmp_image_trans, tmp_all_contours, 2)
    ...
    timeValue = datetime.datetime.fromtimestamp(timeStamp)
    timeFormat = timeValue.strftime('%Y%m%d_%H%M%S_%f')
    f = open(output_dir + "/colonyDetection." +
             timeFormat + ".merge.obj", 'wb')
    pickle.dump(globalOutput, f)
    f.close()
...
def savePCAdata(globalOutput, configure_pool, sample_config_path, pick_dir, output_path):
    ...
    for i in range(sample_config.shape[0]):
        globalOutput.groupID[image_labelIndex_pool[list(sample_config["barcode"])[
            i]]] = list(sample_config["groupID"])[i]
    ...
    for eachGroupID in unique_groupID:
        ...
        groupID_label_list = [globalOutput.image_label[i]
                              for i in groupID_index]
        groupID_image_trans_list = [
            globalOutput.image_trans_corrected[i] for i in groupID_index]
        groupID_image_merge, groupID_image_heightStart = concatenateImages_gray(
            groupID_image_trans_list, groupID_label_list, spacing, fontSize, fontThickness)
        groupID_metadata_list = [globalOutput.all_metadata[i]
                                 for i in groupID_index]
        groupID_metadata_merge = concat_metadata(
            groupID_metadata_list, groupID_image_heightStart)
        ...
        for eachLabel in groupID_label_list:
            tmpCSV = pd.read_csv(
                pick_dir + "/" + eachLabel + "_Metadata_all.csv")
            csvList.append(tmpCSV)
        pick_csv = pd.concat(csvList)
        newDF_dist = {"pickStatus": list(pick_csv["pickStatus"]), "plateBarcode": list(
            pick_csv["plate_barcode"]), "PCA1": list(preprocessed_plates[:, 0]), "PCA2": list(preprocessed_plates[:, 1])}
